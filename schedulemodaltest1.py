...
class CourseSelectModal(discord.ui.Modal):
    def __init__(self):
        super().__init__(title="Select your courses(Development)")
        ...
        for period in range(1,6):
            ...
            self.add_item(discord.ui.Label(text=f"Period {period}",component=PeriodDropdown(self, period)))
            # self.add_item()
...
class PeriodDropdown(discord.ui.Select):
    def __init__(self, parent_view: CourseSelectModal, period: int):
        super().__init__(options=options,placeholder=f"Period {period}", required=False)
        self.period = period
